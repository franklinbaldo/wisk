...
The new LoopRun records the handoff reference and selection reason. The handoff remains active until an outcome successfully consumes or supersedes it.
...
- a run cannot close successfully while its required handoff-environment revalidation remains missing or unresolved.
...
In other words, `wisk check` must be able to complain concretely that a resumed handoff has not documented repository/environment verification.
...
The scheduler can safely issue that command again after a process restart or on its next cadence tick. Persisted OKF state determines whether Wisk resumes existing work, reports a blocker, requires handoff revalidation, or creates a new run.
...
10. process restart does not change the derived next actionable state for unchanged persisted OKF state, explicit inputs, and required external observations.
...
15. `wisk check` reports that revalidation as unsatisfied when it is absent and returns it as the next actionable requirement.
16. documented material drift prevents stale handoff continuation from being treated as current until reassessment is persisted.
17. a resumed run cannot close with missing/unresolved required environment revalidation.
18. multiple zero-prompt handoffs are selected deterministically.
19. uninitialized consumer repositories fail clearly rather than falling through to dogfood layout.
20. every state-changing operation returns one canonical envelope with `state` and either `next`, blockers/remediation, or terminal state.
21. expected cadence/eligibility/resource blocking is represented as typed domain state, not generic exceptions.
22. `next` is derivable from persisted OKF state plus explicit inputs and contract-required external observations.
23. FastMCP exposes the canonical typed start operation and the CLI projects/adapts it without semantic drift.
24. `session start-next` / `wisk_start_next_session` are no longer presented as the golden path.
25. RFC 0005 points to RFC 0006 for canonical entrypoint and explicit-start semantics.
26. tests cover zero-prompt start, explicit task, overrides, RunSpec pinning, live-run resumption, restart recovery, handoff baseline capture, missing revalidation, compatible drift, material drift, close prevention, MCP/CLI parity, and compatibility behavior.

## Consequence

The important architectural change is not the shorter command. It is ownership and recoverability.

A consumer describes what is special about its work. Wisk describes how Wisk operates, persists enough state to resume deterministically, and refuses to silently trust external assumptions that may have gone stale.

Once the repository has encoded its domain-specific contracts, the recurring instruction collapses to:
